...
layui.define(["laytpl", "laypage", "layer", "form", "util"], function(e) {
		...
		u = function(e, a, l, n) {
			if(a != undefined){
				if(a.toString().indexOf('@@')==0){
					if(a.length>2){
						a = a.substring(2)+'<div class="dataErr" title="关联数据已丢失！"></div>';
					}else{
						a = '';
					}
				}
				if(a.toString().indexOf('##')>=0){
					var as = a.split("##");
					if(as.length == 2 && as[0] == as[1]){
						a = as[1];
					}else{
						a = '<span style="color:red;font-size: 11px;">'+as[0]+'</span>→<span style="color:green;">'+as[1]+'</span><div class="dataRisk" title="关联数据发生改变！"></div>';
					}
				}
			}
	...
	I.prototype.config = {
		...
		if(i.startTime = (new Date).getTime(), a.url) {
			...
			a.contentType && 0 == a.contentType.indexOf("application/json") && (d = JSON.stringify(d)), i.loading(), t.ajax({
				...
				success: function(t) {
					...
					try{
						if(unFunctions != undefined && unFunctions.length>0){
							layui.$.each(unFunctions,function(index,ele){
								layui.$('.layui-table-box a[lay-event="'+ele+'"]').remove();
							});
						}
					}catch(e){
		...
		a.layBody.on("click", "." + g, function(e) {
			...
			if(tableCont!=undefined){
				var tt=parseInt(getStyle(tableCont,"height"));
				if(!isNaN(tt)){
					t(".tableBar").css("height",tt+"px").css("padding-top",(tt-16)/2+"px");
					t(".searchCont").css("height",tt+"px");
					var top = t('.tableCont').offset().top;
					t(".right-sidebar").css("height",tt+"px").css('top',top+'px');
				}
			};
		})
	}, d.init = function(e, i) {
		...
		var t = 0,
			...
			l = d.cache[e] || [],
			//TODO 修改
			j = d.configI&&d.configI[e]?d.configI[e].elem.next().find('input[name="layTableCheckbox"]'):{},
			k = 0;
		layui.each(l, function(e, l) {
			k++;
			var m = j[k]?(!j[k].disabled):true;
			return l.constructor === Array ? void i++ : void(l[d.config.checkName] && (t++, m&&a.push(d.clearCacheKey(l))))
		});
		return {
			data: a,
		...
		var t = new I(e);
		//TODO 获得layui.table 基本属性值
		d.configI = d.configI?d.configI:{};
		d.configI[t.config.id] = t.config;
		return c.call(t)
